...
export default function () {
  ...
  // Check the response is in HTML format
  check(response, {
    "response is in HTML format": (r) =>
      r.headers["content-type"] === "text/html; charset=utf-8",
  });

  // Check the response body contains the expected text
  check(response, {
    "response body contains expected text": (r) =>
      r.body.includes("QuickPizza"),
  });

  // Check the response body is in JSON format
  check(response, {
